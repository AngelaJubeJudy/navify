...
const Header: React.FC = () => {
  ...
  const [isForceMobileMenu, setIsForceMobileMenu] = useState(false);
  ...
  useEffect(() => {
    const checkOverflow = () => {
      if (navRef.current) {
        ...
        setIsForceMobileMenu(nav.scrollWidth > nav.clientWidth);
      }
    };
    checkOverflow();
    window.addEventListener('resize', checkOverflow);
    return () => window.removeEventListener('resize', checkOverflow);
  }, [navigation.map(item => item.name).join('')]);
